...
def repo_relative(p: Path) -> str:
    """Path as written into the tables: relative to the repo whenever it is inside it.

    The `path` column is committed to `data/processed/blocks.csv`, so it must not
    depend on where the repository happens to live. It used to: DEFAULT_ROOT is
    absolute, so running the documented command wrote
    `/home/<someone>/bci-ews-research/data/raw/...` into a tracked file, while the
    committed copy held repo-relative paths because it had been generated by
    passing `--root data/raw` by hand. The two never matched, and regenerating the
    file on any machine produced a diff that was pure environment.

    A path outside the repository is left absolute, since there is nothing
