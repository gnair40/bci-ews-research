...
Nine gates, a mutation check and a test suite — **one command**:

```
python3 tools/check_all.py
```

Run it before committing anything, never after, or you end up with regenerated
files that disagree with the code. `--quick` skips the slow mutation check;
`--list` shows what it would run. It keeps going after a failure instead of
stopping at the first, because stopping early hides how much is wrong.

| | Gate | Asks |
|---|---|---|
| | `python3 -m unittest discover -s tests` | 50 tests, every one derived from a mistake this project actually made |
| | `tools/mutation_check.py` | do those tests catch anything? |
| 31 | `scripts/31_verify_claims.py` | every headline number, recomputed |
| 55 | `scripts/55_reproducibility_audit.py` | can the pipeline be rebuilt from what is committed? |
| 56 | `scripts/56_claims_register.py` | does every claim have a registered source? |
| 61 | `scripts/61_statistical_hygiene.py` | has a practice we committed to avoiding come back? |
| 65 | `scripts/65_log_coverage.py` | did work happen that the log does not record? |
| 69 | `scripts/69_command_check.py` | would a reader's first command actually work? |
| 77 | `scripts/77_cross_reference.py` | do the documents agree with each other and the code? |
| 79 | `scripts/79_preregistration_freeze.py` | is the frozen preregistration still frozen, and every edit to it disclosed? |
| 80 | `scripts/80_remote_freshness.py` | is this working copy's view of the repository current, or is a local ref stale? |
| | `physical/code/monitor.py --selftest` | does the physical phase's pipeline run end to end? |

**Each of the last four exists because something got through.**
`69_command_check.py` was missing from the README's list until 20 September
2026, which is exactly the way a gate stops being run — and is why the list is
now a script. `77_cross_reference.py` was added on 21 September after two
documents were found disagreeing about the same number for two weeks: every
other gate checks a figure against the data file it came from, and nothing was
checking that two documents agree. `79_preregistration_freeze.py` was added on
24 September, the day the preregistration was frozen, because a header saying
"do not edit this file" is not a lock and it would be dishonest to rely on one.
`80_remote_freshness.py` was added the same night, after a claim about the
`main` branch was written into four documents and committed — a claim drawn
from a *local* ref that had never been fetched and was five weeks stale. Every
other gate compares the repository to itself, and from the inside a stale ref
