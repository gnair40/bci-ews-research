...
## 17 September 2026 — Chasing one stale figure found three more reproducibility bugs

Went back to a loose end noted weeks ago and never chased: `scripts/23` produced
a different `13_auc_by_fault_mode.png` than the committed one. It turned out to
be the visible end of a pattern.

**The figure.** Every number in it was right. What was missing was the second
line of the y-axis label, `(window-level AUC)` — which is exactly the disclosure
that those bars are a window-level quantity, the thing the claims register warns
must not be read as an episode-level result. The stale copy omitted precisely the
caveat that had been added to stop it being misread. Cause: figures were
gitignored while the project was being written, so the copies on disk drifted
from the scripts. The label went in at commit 110; figures started being tracked
at 111, which froze whatever happened to be on disk. Wrong for thirteen days.

Regenerating the other figure scripts to see how far it went turned up three
more problems, none of which had anything to do with figures.

**`blocks.csv` held absolute paths.** The committed copy has repo-relative paths
in its `path` column. Running the documented command writes
`/home/user/bci-ews-research/data/raw/...` instead, because `DEFAULT_ROOT` is
absolute and has been since 25 August. The committed file had been generated by
passing `--root data/raw` by hand. So the documented command never reproduced the
committed file, on any machine, and the file leaked whoever's home directory it
was made in. Fixed by writing the path relative to the repo whenever it is inside
it. `blocks.csv` now regenerates byte-identically.

**`ews_control_results.json` was made with a flag nobody wrote down.** It records
`n_surrogates: 300`; the script's default is 500 and always has been. Same shape
of problem: an artifact produced by an undocumented invocation. Regenerated with
the documented command. Every conclusion is identical — `passed` stays true, and
every indicator fires or does not fire exactly as before. Only the p-values move,
which is what more surrogates does: 0.0067 → 0.0100 on saddle-node variance,
0.1300 → 0.0860 on rising-noise AR(1).

**The one that actually mattered: regenerating destroyed hand-written findings.**
`DATASET_EXPLORATION.md` sections 6, 9 and 10 were written by hand after Phases
1–2 and carry real results — the change point at T11 day 758 agreed by three
methods at p = 0.0018, mean firing rate falling 56.5%, the detrending omission
that produced p = 0.0002, the list of analyses the dataset cannot support. The
script still held the original "Requires your judgement" placeholders, so running
it put the questions back and threw the answers away. I did exactly that while
investigating, and had to restore from git.

**This is the second time.** The first was the correction block in
`ACHIEVABILITY.md`, lost the same way in August. The fix then was to move the
prose into the script that emits it; the same fix is now applied here, as
`SECTION_6`, `SECTION_9` and `SECTION_10`. That it happened twice says the
pattern is the problem, not the incident: any report that is both generated and
hand-edited will eventually be regenerated by someone who does not know it was
edited. A report should be entirely generated or entirely written, and where it
has to be both, the hand-written part belongs in the generator.

**What I built.** `tools/figure_freshness.py` redraws the figures and compares.
It is deliberately not one of the five gates, because the cheap checks do not
work: modification times are useless since git sets them to checkout time, and
comparing commit dates fails on the exact case that motivated this — the figure's
last commit is *newer* than the script's, so a date check calls it fresh. Only
redraw-and-compare works, and that is too slow to run before every commit. It
restores the committed figures afterwards rather than overwriting them; it
reports, it does not decide.

**What is still unchecked.** `09_reproduce_mindful.py` was killed mid-run when I
stopped the sweep, so its figure has not been compared. `--all` covers it and the
other raw-data scripts. Saying so rather than implying the sweep was complete.
