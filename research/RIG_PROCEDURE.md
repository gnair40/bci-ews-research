...
Regenerated by `python3 scripts/66_window_spacing.py`, and checked by four
entries in the claim verifier (register claim **C18**).
...
> **Corrected 6 September 2026.** An earlier version of this section said a new
> loader had to be written as a new numbered script. It does not.
> `scripts/67_rig_format_check.py` established that if the rig writes plain
> `.mat` files in the deposit's own folder layout, **the archived loader reads
> them with no new code at all.** That is a simpler and stronger answer, so the
> instruction below replaces the one that was here.

**Procedure 5.1. Have the capture program write `.mat` files, not `.npy`.**
Per block, create `data/raw_rig/RIG/day_<session>/block_<n>/` holding three
files, written with `scipy.io.savemat`. This is exactly the layout the MINDFUL
deposit uses, which is why nothing downstream has to change:

| File | Field | Shape | Contents |
|---|---|---|---|
| `data.mat` | `nctx` | `(n_bins, 384)` | Region-mean intensities from `capture.py` |
| `info.mat` | `cursorPos` | `(n_bins, 2)` | Always `(0, 0)` |
| | `targetPos` | `(n_bins, 2)` | `(cos φ, sin φ)` for that frame's heading |
| | `angleErrorPerTrial` | `(n_trials,)` | Median angular error per constant-heading segment |
| | `trialSuccess`, `timeToTarget`, `pathEfficiency`, `orthChanges` | `(n_trials,)` | Per-trial measures |
| | `percentCorrect` | scalar | Block summary |
| `task.mat` | `startStops` | `(n_trials, 2)` | **MATLAB 1-based** start and stop bin per trial |
| | `name` | string | Task label, e.g. `rigCosineField` |
| | `excludeTrials`, `useClick` | `(n_trials,)` | All `False` for the rig |

**Two things that will silently break this if you get them wrong.**
`startStops` is **1-based**, because MATLAB counts from 1 and the loader
converts. Off-by-one here shifts every trial by a bin and corrupts the
neural/behavioural alignment. And `savemat` **silently drops any field whose
name begins with an underscore** — it warns, but the field simply does not
appear in the file.
...
```
This fabricates a synthetic block in the layout above, loads it with the
**unmodified** archived loader, and confirms the reference decoder's required
...
recordings would have silently not existed. The pattern now also admits `RIG`.
Naming the rig `T99` would have needed no code change and would have been the
wrong call: commit `7102ff6` exists specifically to establish that
"participant" never means a recruited person, and a folder labelled
"participant T99" undoes that in the one place a judge is most likely to look.
