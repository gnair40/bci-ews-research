...
def load_config(cfg: str) -> dict | None:
    ...
    if meta.exists():
        ...
        out["synthetic"] = bool(m.get("synthetic"))
...
def main() -> int:
    ...
    # ANY synthetic configuration taints the comparison, because the report's
    # whole subject is whether the four agree. A mix of real and fake is worse
    # than all fake, not better.
    any_synthetic = any(r.get("synthetic") for r in rows)
    result["synthetic"] = any_synthetic
    ...
    L = ["# P-7 — Is any of this a property of one box?\n",
         *M.provenance_banner({"synthetic": any_synthetic}),
